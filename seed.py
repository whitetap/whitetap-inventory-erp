...
def seed_data():
    with app.app_context():
        ...
        db.session.commit()
        db.session.add(FormulaIngredient(result_color_id=red_carpet.id, pigment_id=bases[0].id, quantity=100.0))
        db.session.add(FormulaIngredient(result_color_id=red_carpet.id, pigment_id=pigments[0].id, quantity=15.0))
        ...
        print('🎉 Seed complete! Visit http://localhost:5005')
